...
I=1
for LON0 in `seq 0 10 360`
do

    echo "$SCRIPT [`print_time`]: working on frame centered on lon=$LON0" | tee -a $LOG_FILE

    # Set frame name
    IZERO=`echo $I | awk '{printf("%05d"),$1}'`
    PSFILE=frame_$IZERO.ps
...
ffmpeg -loglevel warning -framerate 24 -y -i "frame_%05d.png" -vf "scale=trunc(iw/2)*2:trunc(ih/2)*2" -vcodec libx264 -pix_fmt yuv420p out.mp4
